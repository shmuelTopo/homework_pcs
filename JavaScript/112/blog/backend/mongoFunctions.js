...
const { post, contains } = require("jquery");
...
module.exports.getPosts = async ({ limit, lastpostid }, baseurl) => {
  ...
  if(lastpostid) {
    try {
      post = await posts.findOne({_id: ObjectId(id)}, { projection: {datetime: 1, _id: 0}});
      if(!post) {
        throw new Error();
      }
    } catch(e) {
      const err = new Error(`can't find blog post with id ${lastpostid}`);
      err.statusCode = 404;
      throw err;
    } 
    query = { 
  ...
  if(postsResponse.nextPostsCount) {
    ...
    postsResponse.nextPageReq = `${baseurl}/posts?limit=${limit}&lastpostid=${lastPostId}`;
  }
