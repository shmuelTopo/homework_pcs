...
export default function User({user, userClicked}) {
    ...
    return (
        <div onClick={() => navigate(`/posts/${user.id}`)} key={user.id} userid={user.id} className="userDetails userDetails text-center bg-base-content/20 hover:bg-base-content/30 p-4 rounded-xl cursor-pointer"> 
